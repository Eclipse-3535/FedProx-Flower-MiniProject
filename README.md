...
3. **消融实验**：通过调整近端项系数 $\mu$，对比了 $\mu=0$ (退化为 FedAvg)、$\mu=0.01$ 和 $\mu=0.1$（过大的惩罚导致欠拟合）三种情况下的收敛曲线。
...
python server.py
*(运行结束后，将在当前目录生成 `results_mu_0.0.json` 文件)*

### 2. 运行 FedProx ($\mu=0.01$)
将 `server.py` 中第 10 行修改为 `MU = 0.01`，然后运行：
python server.py
*(运行结束后，将在当前目录生成 `results_mu_0.01.json` 文件)*

### 3. 生成对比图表
确保上述两个 JSON 文件都已生成，运行：
python plot.py
*(该脚本将读取 JSON 文件，并生成 `fedprox_vs_fedavg.png` 包含 Accuracy 和 Loss 的对比曲线图)*
...
*   **本地迭代次数 (Epochs)**：设置为 5，以刻意放大本地模型偏离全局模型的程度，从而凸显 FedProx 的优势。